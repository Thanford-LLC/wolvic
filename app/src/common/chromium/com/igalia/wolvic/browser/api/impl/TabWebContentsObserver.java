...
public class TabWebContentsObserver extends WebContentsObserver {
    private static final int GLYPHEW_MSE_READY_PROJECTION = 9;
    ...
    public void mediaProjectionChanged(int projectionType, int stereoMode) {
        ...
        if (delegate instanceof com.igalia.wolvic.browser.Media) {
            com.igalia.wolvic.browser.Media media = (com.igalia.wolvic.browser.Media) delegate;
            if (projectionType == GLYPHEW_MSE_READY_PROJECTION) {
                media.markGlyphewMseReady();
            } else {
                media.setProjection(projectionType, stereoMode);
            }
        }
    }
}
