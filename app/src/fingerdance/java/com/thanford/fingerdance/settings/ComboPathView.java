 ...
 * <ul>
 *   <li>Path arcs from node to node along the ring (shorter-arc direction)</li>
 *   <li>Start node: filled yellow dot; subsequent nodes: hollow ring</li>
 *   <li>Multi-hit: 240° loop arc outside the node + "N×" badge at loop center</li>
 *   <li>Serpentine ABABABAB… paths: converging quadratic bezier curves,
 *       opacity 0.14 (oldest) → 1.0 (newest)</li>
 *   <li>Unbound (null path): dim ring only</li>
 * </ul>
 *
 * <p>All drawing is clipped to the disc boundary, so loop arcs that extend
 * beyond the ring are cleanly trimmed.
 *
 * <p>Node → angle mapping (Android canvas, 0° = right, clockwise):
 * <pre>
 *   7(225°) 2(270°) 9(315°)
 *   4(180°)   5     6(  0°)
 *   1(135°) 8( 90°) 3( 45°)
 * </pre>
 * This maps joystick directions to visual positions: push up = node 2 = top.
 */
public final class ComboPathView extends View {

    // Android canvas degrees (0 = right, clockwise).
    ...
    /** Set the combo path; null or empty shows unbound state (dim ring only). */
        ...
        if (mPath != null) {
            ...
            if (serpentine) {
                ...
                for (int i = 0; i + 1 < deduped.length; i++) {
                    drawPathArc(canvas, deduped[i], deduped[i + 1], ringRect, mArcPaint);
                }
            }

            // --- Multi-hit loop arcs + count badges ---
            ...
            // --- Node dots (start = filled, rest = hollow) ---
    ...
    private void drawPathArc(Canvas canvas, int from, int to, RectF ring, Paint paint) {
        if (!validNode(from) || !validNode(to)) return;
        float start = NODE_ANGLE_DEG[from];
        float diff  = ((NODE_ANGLE_DEG[to] - start) % 360f + 360f) % 360f;
        // Prefer the shorter arc; equal (180°) defaults to CW.
        float sweep = diff <= 180f ? diff : diff - 360f;
    ...
    private void drawLoopArc(Canvas canvas, int node,
        ...
        float dist = ringR + loopR * 0.85f;
        float lcx  = cx + dist * (float) Math.cos(rad);
        float lcy  = cy + dist * (float) Math.sin(rad);
        // Gap faces inward (toward ring center); arc sweeps 240° outward.
    ...
    private void drawBadge(Canvas canvas, int node,
                           ...
                           float sp, int count) {
        float rad  = (float) Math.toRadians(NODE_ANGLE_DEG[node]);
        float dist = ringR + loopR * 0.85f; // draw inside the loop circle center
        float bx   = cx + dist * (float) Math.cos(rad);
        float by   = cy + dist * (float) Math.sin(rad);
    ...
    private void drawSerpentineArcs(Canvas canvas, int[] path,
        ...
        for (int i = 0; i < nArcs; i++) {
            ...
            float offset = ringR * (0.55f - 0.4f * t); // converge large → small
            int   side   = (i % 2 == 0) ? 1 : -1;
    ...
    /** Returns per-node max consecutive hit count (indices 1–9). */
    ...
    /** True when path is A B A B A B… (length ≥ 4, A ≠ B). */
    ...
    /** Removes consecutive duplicate nodes, e.g. [2,2,4] → [2,4]. */
    @NonNull
